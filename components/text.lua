...
- x (number): 文本的 x 坐标
- y (number): 文本的 y 坐标
- font_size (string, 可选): 字号，默认 "p"，可选 "h1"-"h6" 或 "p"
- fade_in (boolean, 可选): 文本是否淡入显示
- fade_out (boolean, 可选): 文本是否淡出隐藏
- wrap_width (number, 可选): 自动换行宽度（像素），nil 表示不换行
- align (string, 可选): 对齐方式，如 "left"、"center"、"right"
